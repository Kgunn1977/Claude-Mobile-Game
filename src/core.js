...
(function (root, factory) {
  ...
  const CONFIG = {
    ...
    // colony-health weights (sum ~1 before threat penalty)
    chWeights: { vitals: 0.50, food: 0.16, water: 0.10, firewood: 0.10, pop: 0.14 },
    chCushionTargetDays: 20,    // days of supply that counts as "full" cushion

    // turn length: [colonyHealth <, hours]
    turnBands: [[15, 1], [30, 24], [50, 168], [70, 720], [88, 2160], [101, 8640]],
  ...
  function makeColonist(rng, locId, origin) {
    ...
    return {
      ...
      stamina: 100, hunger: 80, warmth: 65, health: startH, happiness: 72,
  ...
  function computeColonyHealth(s) {
    ...
    const w = CONFIG.chWeights;
    const vit = live.reduce((a, c) => a + (c.hunger + c.warmth + c.health + c.happiness) / 4, 0) / live.length;
    const days = r => (s.store[r] || 0) / Math.max(1, live.length);
    const cush = d => clamp(d / CONFIG.chCushionTargetDays, 0, 1) * 100;
    const foodC = cush(totalFood(s) / Math.max(1, live.length));
    const waterC = cush(days("water"));
    const fireC = season(s) === "Winter" ? cush(days("firewood")) : 100;
    const popC = clamp(live.length / CONFIG.START_FOUNDERS, 0, 1.2) * 83;
    let ch = vit * w.vitals + foodC * w.food + waterC * w.water + fireC * w.firewood + popC * w.pop;
