...
(function () {
  ...
  function openBuildSheet() {
    ...
    Object.keys(C.BUILDINGS).forEach(id => {
      ...
      html += `<div class="opt ${afford ? "" : "disabled"}" data-id="${id}"><div class="oi">${b.name} <span class="muted" style="font-size:11px">— ${defOutputText(b)}</span><small>needs ${mats} · ${b.build.work} work${b.skill ? " (" + cap2(b.skill) + ")" : ""}</small></div><button class="btn sm ${afford ? "primary" : ""}" ${afford ? "" : "disabled"}>Build</button></div>`;
    });
    openSheet(html);
